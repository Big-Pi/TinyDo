...
- (IBAction)alarmClick:(UIButton *)sender {
    [self.delegate editableCellDidAlarmClick:self];
}

- (IBAction)priorityClick:(UIButton *)sender {
    [self.delegate editableCellDidPriorityClick:self];
}
...
-(void)setInsertOrEdit:(BOOL)insertOrEdit anim:(BOOL)anim{
    if(anim){
        [UIView animateWithDuration:0.35 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            self.alarm.transform=insertOrEdit ? CGAffineTransformMakeScale(1.2, 1.2) : CGAffineTransformMakeScale(0.0, 0.0);
            self.priority.transform =insertOrEdit ? CGAffineTransformMakeScale(1.2, 1.2) :CGAffineTransformMakeScale(0.0, 0.0);
